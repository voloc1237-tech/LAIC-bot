...
Финальная версия с поддержкой секретов NASA и сокращёнными логами.
"""
...
class IonosphereCollector:
    ...
    def __init__(self, cache_dir="data/ionex_cache"):
        ...
        # 🔥 КЛЮЧЕВОЕ: СЧИТЫВАЕМ СЕКРЕТЫ ИЗ ОКРУЖЕНИЯ
        ...
        if self.username and self.password:
            logger.info("🛰️ Ионосфера: NASA аутентификация настроена")
        else:
            logger.info("🛰️ Ионосфера: кэш IONEX в data/ionex_cache (без NASA)")
    ...
    def _download_ionex(self, date: datetime):
        """Скачивает IONEX с CDDIS (с секретами) или зеркал."""
        ...
        # Формируем URL для CDDIS с аутентификацией
        long_name = (
            "COD0OPSFIN_"
        ...
        # 1. CDDIS с аутентификацией (используем секреты!)
        if self.username and self.password:
            for attempt in range(self.MAX_RETRIES):
                try:
                    auth = HTTPBasicAuth(self.username, self.password)
                    r = requests.get(cddis_url, auth=auth, timeout=60, stream=True)
                    r.raise_for_status()
                    ...
                    if e.response.status_code == 403:
                        logger.warning("⚠️ CDDIS 403: проверьте NASA_USERNAME/PASSWORD")
            ...
            if any(c.isalpha() for c in line):
                if "EPOCH OF CURRENT MAP" in line and pending:
                    ...
                    try:
                        ...
                        current_lat_idx = int(np.argmin(np.abs(lats - lat_val)))
    ...
    def fetch_tec_for_point(self, lat, lon, start_time, end_time):
        ...
        if not all_dfs:
            logger.debug(f"⚠️ Нет TEC для ({lat:.2f}, {lon:.2f}) → синтетика")
        ...
        logger.debug(f"🛰️ TEC: {len(df_combined)} записей для ({lat:.2f}, {lon:.2f})")
        return df_combined

    def fetch_for_event(self, event_time, lat, lon, days_before=7, days_after=3):
        """Сбор TEC + анализ аномалий (с кэшем)."""
        ...
        result = {
            ...
            "mean_bg": mean_bg,
            "std_bg": std_bg,
            "level": "critical" if abs(z_max) > 3.0 or abs(z_min) > 3.0 else "normal",
            "n_historical_years": 1,
            "year_over_year_change": round(((target_df["tec_value"].mean() - mean_bg) / mean_bg) * 100 if mean_bg > 0 else 0, 2),
            "details": details,
        }
        self._event_cache[cache_key] = result
        return result
