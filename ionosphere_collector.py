...
class IonosphereCollector:
    ...
    def __init__(self, cache_dir="data/ionex_cache"):
        ...
        # Проверяем наличие gnssanalysis ПРИ СОЗДАНИИ ОБЪЕКТА
    ...
    def _check_gnssanalysis(self):
        ...
        try:
            ...
            logger.info("✅ gnssanalysis загружена и готова к работе")
            return True
        except ImportError as e:
            logger.warning(f"⚠️ gnssanalysis не найдена: {e}")
            return False
        except Exception as e:
            logger.warning(f"⚠️ Ошибка при импорте gnssanalysis: {e}")
            return False
    ...
    def fetch_tec_for_point(self, lat, lon, start_time, end_time) -> pd.DataFrame:
        ...
        if self.gnss_available:
            try:
                ...
                logger.debug(f"Ошибка реальных данных: {e}")
        
        # Если реальные не получены — используем синтетику
        logger.debug(f"🔄 Использую синтетические TEC для ({lat:.2f}, {lon:.2f})")
    ...
    def fetch_for_event(self, event_time, lat, lon, days_before=7, days_after=3) -> dict:
        ...
        if bg_df.empty:
            details = "Нет данных фона (прошлый год) для расчета аномалии"
